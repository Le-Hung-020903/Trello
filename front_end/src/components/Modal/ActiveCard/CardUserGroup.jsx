...
function CardUserGroup({ cardMemberIds = [], onUpdateCardMembers }) {
  ...
  // Lấy activeBoard từ redux ra để mục đích là lấy được toàn bộ
  // info thành viên của board thông qua field FE_allUsers
  const board = useSelector(selectCurrentActiveBoard)

  // Thành viên trong card sẽ phải là tập con của board nên tạo 1 mảng FE_CardMembers
  // chứa đầy dủ info user để hiển thị ra ngoài giao diện, bởi
  // mặc định trong card chỉ lưu mỗi id của user (cardMemberIds)
  ...
  const handleUpdateCardMembers = (user) => {
    // Tạo một biến incomingMemberInfo để gửi lên BE, với 2 info là userID và action (thêm or xoá)
    ...
    // Gọi lên thằng cha (active card) để thêm vào db
